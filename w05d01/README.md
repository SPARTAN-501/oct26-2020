...
-- using AS to alias the count result
...
```sql
SELECT album_name, artist_name, COUNT(songs.id)
FROM albums
JOIN songs ON albums.id = album_id
...
```sql
SELECT album_name, artist_name, AVG(rating) AS avg_rating
FROM albums
...
SELECT album_name, 
  song_name,
  rating, 
  (SELECT AVG(rating) FROM songs WHERE album_id = albums.id) AS album_avg
FROM albums
